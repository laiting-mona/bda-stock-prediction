...
print("=" * 60)
print("Phase 3 Moving Backtest - SVM Model (v2)")
print("=" * 60)
...
print(f"Total backtest days : {len(backtest_dates)} ({backtest_dates[0]} ~ {backtest_dates[-1]})")
print(f"Days with features  : {sum(1 for d in backtest_dates if d in feat_dates)}")
print(f"Neutral days (skip) : {sum(1 for d in backtest_dates if d not in feat_dates)}")
...
for test_date in backtest_dates:
    ...
    if len(test_feat) < MIN_ARTICLES:
        results.append({'date': test_date, 'signal': 'skip',
                        'reason': 'neutral or insufficient articles',
                        'trade_return': 0.0,
...
print(f"Days traded   : {len(traded)} ({len(traded)/len(result_df)*100:.1f}%)")
print(f"Days skipped  : {len(result_df) - len(traded)}")
if len(traded) > 0:
    print(f"Win rate      : {len(win_trades)/len(traded)*100:.1f}%")
print(f"Cumul. return : {result_df['trade_return'].sum()*100:.2f}%")
print(f"Buy & Hold    : {result_df['actual_return'].sum()*100:.2f}%")

csv_path = OUTPUT_DIR / "svm_backtest_results.csv"
...
ax.plot(result_df['date'], result_df['cum_strategy'],
        label='SVM Strategy', color='darkorange', linewidth=2)
ax.plot(result_df['date'], result_df['cum_bh'],
        label='Buy & Hold', color='gray', linewidth=1.5, linestyle='--')
ax.axhline(0, color='black', linewidth=0.8, linestyle=':')
ax.set_title('SVM Backtest (v2) - Cumulative Return vs Buy & Hold', fontsize=13)
ax.set_xlabel('Date')
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "svm_cumulative_return.png", dpi=150)
plt.close()
...
if len(traded) > 0:
    ...
    ax2.bar(monthly['month'], monthly['count'], color='darkorange')
    ax2.set_title('SVM Monthly Trade Count (v2)')
    ax2.set_xlabel('Month')
    ax2.set_ylabel('Trade Days')
    plt.xticks(rotation=30)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "svm_monthly_trades.png", dpi=150)
    plt.close()
