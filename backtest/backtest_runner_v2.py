"""
Backtest runner v2: RF_v2, NB_v2, XGBoost_v2
Features: TF-IDF (train-only chi2, sigma=0.8%) + technical indicators
More test dates than v1 due to lower sigma threshold.
Output: results/backtest/v2/
"""
...
class SoftVotingEnsemble:
    """Mirror of models/ensemble/ensemble_v2_train.py — needed for pickle deserialization."""
...
DATA_DIR = ROOT / "data"
OUT_DIR  = ROOT / "results" / "backtest" / "v2"
...
MODELS = {
    ...
    "Ensemble_v2": ROOT / "models/ensemble/ensemble_v2_model.pkl",
}
...
SPLIT               = 0.8
COLORS              = {"RF_v2": "steelblue", "NB_v2": "darkorange",
                       "XGBoost_v2": "forestgreen", "Ensemble_v2": "purple"}
...
def plot_cumulative(sim_map, window, out_path):
    ...
    ax.set_title(f"v2 Cumulative Return — {window}-Day Window (threshold=0.50)")
...
def plot_threshold_comparison(sumdf, model_name, out_path):
    ...
    plt.suptitle(f"{model_name} v2: Threshold Comparison", y=1.02)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150); plt.close()
...
def plot_monthly_heatmap(daily_list, out_path):
    ...
    ax.set_title("v2 Monthly Trade Signal Count (threshold=0.50)")
...
def main():
    print("=" * 60)
    print("v2 Models Backtest (TF-IDF + Technical Features)")
    print("=" * 60)
    ...
    # Skip models whose .pkl files don't exist yet
    ...
    sumdf.to_csv(OUT_DIR / "backtest_v2_summary.csv", index=False, encoding="utf-8-sig")
    ...
    for w in WINDOWS:
        ...
        for mn in active_models:
            ...
            sim_map[mn] = sim
        plot_cumulative(sim_map, w, OUT_DIR / f"v2_cumulative_return_{w}d.png")

    for mn in active_models:
        plot_threshold_comparison(sumdf, mn, OUT_DIR / f"{mn}_threshold_comparison.png")

    plot_monthly_heatmap(list(daily_thr050.values()),
                         OUT_DIR / "v2_monthly_signal_heatmap.png")

    err_df = pd.concat(errors, ignore_index=True)
    err_df.to_csv(OUT_DIR / "v2_error_analysis.csv", index=False, encoding="utf-8-sig")

    print("\n" + "=" * 70)
    print("v2 FINAL SUMMARY  (threshold=0.50)")
    print("=" * 70)
