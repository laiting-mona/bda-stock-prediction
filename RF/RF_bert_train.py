"""
Random Forest trained on BERT (bert-base-chinese) 768-dim CLS embeddings.
...
model = RandomForestClassifier(**best_p, n_estimators=300, random_state=42, n_jobs=-1)
...
ax.set_title("RF (BERT) Confusion Matrix")
plt.tight_layout()
plt.savefig(CM_DIR / "rf_bert_confusion_matrix.png", dpi=150)
plt.close()

# ── Save model ────────────────────────────────────────────────────────────────
with open(MODEL_DIR / "RF_bert_model.pkl", "wb") as f:
    pickle.dump(model, f)
print(f"RF_bert_model.pkl saved → {MODEL_DIR / 'RF_bert_model.pkl'}")
