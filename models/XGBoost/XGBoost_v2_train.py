"""
XGBoost v2: combined TF-IDF (v2, train-only chi2) + technical features.
TimeSeriesSplit CV, CalibratedClassifierCV (sigmoid).
Separate from original XGBoost_model.pkl — outputs XGBoost_v2_model.pkl.
"""
...
ax.set_title("XGBoost v2 (TF-IDF+Tech, Calibrated) Confusion Matrix")
plt.tight_layout()
plt.savefig(CM_DIR / "xgboost_v2_confusion_matrix.png", dpi=150)
plt.close()

# ── Save model ────────────────────────────────────────────────────────────────
with open(MODEL_DIR / "XGBoost_v2_model.pkl", "wb") as f:
    pickle.dump(cal_xgb, f)
print(f"XGBoost_v2_model.pkl saved -> {MODEL_DIR / 'XGBoost_v2_model.pkl'}")
