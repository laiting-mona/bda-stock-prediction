...
scale_pos_weight = round(float(counts[0]) / float(counts[1]), 4)
print(f"scale_pos_weight (neg/pos): {scale_pos_weight}")

# ── Stage 1：固定 100 棵樹，掃最佳超參數 ─────────────────────────────────────
param_grid = {
    "n_estimators":     [100],
    "max_depth":        [3, 5, 7],
...
print(f"\nStage1 best params (excl. n_estimators): {best_params}")
print(f"Stage1 best CV Macro F1: {grid.best_score_:.4f}")

# ── Stage 2：最佳超參數 + 300 棵樹重新訓練 ───────────────────────────────────
