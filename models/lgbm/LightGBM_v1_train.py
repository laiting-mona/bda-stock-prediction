"""
LightGBM v1: 322-dim features (300 TF-IDF + 22 tech from v3).
LightGBM advantages over XGBoost: faster training, better handling of
high-cardinality features, native categorical support, leaf-wise growth.
  - is_unbalance=True handles class imbalance natively
  - Decision threshold optimised on held-out validation fold
  - No Calibration needed — LightGBM probabilities are already well-calibrated
Output: LightGBM_v1_model.pkl
"""
...
lgb_tmp = LGBMClassifier(**best_p, n_estimators=300, random_state=42,
                          n_jobs=-1, verbosity=-1, is_unbalance=True)
lgb_tmp.fit(X_tr2, y_tr2)
val_prob   = lgb_tmp.predict_proba(X_val)[:, 1]
...
# ── Refit on full train ────────────────────────────────────────────────────────
lgb_final = LGBMClassifier(**best_p, n_estimators=300, random_state=42,
                            n_jobs=-1, verbosity=-1, is_unbalance=True)
lgb_final.fit(X_train, y_train)

prob_test   = lgb_final.predict_proba(X_test)[:, 1]
...
ax.set_title(f"LightGBM v1 (T={best_t:.2f}) Confusion Matrix")
plt.tight_layout()
plt.savefig(CM_DIR / "lgbm_v1_confusion_matrix.png", dpi=150)
plt.close()

# ── Save ──────────────────────────────────────────────────────────────────────
wrapper = LGBMv1Wrapper(lgb_final, best_t, cv_f1)
with open(MODEL_DIR / "LightGBM_v1_model.pkl", "wb") as f:
    pickle.dump(wrapper, f)
print(f"\nLightGBM_v1_model.pkl saved  (T={best_t:.2f}) -> {MODEL_DIR / 'LightGBM_v1_model.pkl'}")
