...
val_prob   = rf_tmp.predict_proba(X_val)[:, 1]
...
ax.set_title(f"RF v5 (balanced_subsample, T={best_t:.2f}) Confusion Matrix")
plt.tight_layout()
plt.savefig(CM_DIR / "rf_v5_confusion_matrix.png", dpi=150)
plt.close()

# ── Save ──────────────────────────────────────────────────────────────────────
wrapper = RFv5Wrapper(rf_final, best_t, cv_f1)
with open(MODEL_DIR / "RF_v5_model.pkl", "wb") as f:
    pickle.dump(wrapper, f)
print(f"\nRF_v5_model.pkl saved  (T={best_t:.2f}) -> {MODEL_DIR / 'RF_v5_model.pkl'}")
