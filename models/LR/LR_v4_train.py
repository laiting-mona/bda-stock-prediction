"""
Logistic Regression v4: L2-regularised on enhanced 613-dim day-level features.
LR is well-calibrated, naturally balanced, and strong on text + tech combined.
Includes confidence-stratified accuracy and optimal threshold finder.
Output: models/LR/LR_v4_model.pkl
"""
...
print("Loading enhanced day-level v4 features...")
meta_df = pd.read_csv(DATA_DIR / "processed" / "tsmc_day_features_v4.csv",     encoding="utf-8-sig")
text_df = pd.read_csv(DATA_DIR / "processed" / "tsmc_day_text_enhanced_v4.csv", encoding="utf-8-sig")
tech_df = pd.read_csv(DATA_DIR / "processed" / "tsmc_day_tech_enhanced_v4.csv", encoding="utf-8-sig")

y     = meta_df["label"].astype(int).values
X_all = np.hstack([text_df.values, tech_df.values])   # (267, 613)
...
ax.set_title(f"LR v4 (day-level, T={best_t:.2f}) Confusion Matrix")
plt.tight_layout()
plt.savefig(CM_DIR / "lr_v4_confusion_matrix.png", dpi=150)
plt.close()
...
wrapper = LRv4Model(model, best_t, cv_f1)
with open(MODEL_DIR / "LR_v4_model.pkl", "wb") as f:
    pickle.dump(wrapper, f)
print(f"\nLR_v4_model.pkl saved  (T={best_t:.2f}) -> {MODEL_DIR / 'LR_v4_model.pkl'}")
